    ...
    $.fn.FKPageTransitions = function (options) {
        ...
        var onTouchStart = function (e) {
            ...
            if (slider.working) {
                ...
                var touchIgnoreFlag = checkTouchIgnoreFlag(e.target);
                if (!touchIgnoreFlag) {
        ...
        /**
         * 检查滑动时是否在忽略滑动翻页DOM元素内
         */
        var checkTouchIgnoreFlag = function (target) {
            var touchIgnoreFlag = false;
            if (slider.settings.touchIgnoreDomClass) {
                var touchIgnoreDoms = document.getElementsByClassName(slider.settings.touchIgnoreDomClass);
                $.each(touchIgnoreDoms, function (touchIgnoreDomIndex, touchIgnoreDom) {
                    if (touchIgnoreDom.contains(target)) {
                        touchIgnoreFlag = true;
                        return;
                    }
                });
            }
            return touchIgnoreFlag;
        };
        /**
         * 设置动画效果
         *
